...
async function requestHandler(config: InternalAxiosRequestConfig & RequestConfigExtra): Promise<InternalAxiosRequestConfig> {
  // The URL before processing the request
  if (
    import.meta.env.DEV
    ...
    //  Replace the url request prefix baseUrl
  ...
  config.headers.set('Accept-Language', locale.value ?? 'en-US')
  if (config.loading)
...
function errorHandler(error: AxiosError): Promise<any> {
  ...
  if (error.response) {
    ...
    if (status === 401) {
      ...
      /**
       * This handles the logic of clearing user information and tokens, which will be expanded later.
       */
      token.value = null
