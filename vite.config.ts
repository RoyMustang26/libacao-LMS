...
const baseSrc = fileURLToPath(new URL('./resources/js/LMS', import.meta.url));
...
export default ({ mode }: ConfigEnv): UserConfig => {
  ...
  return {
    ...
    build: {
      ...
      // Keep ONLY rollupOptions — safe for Docker
      rollupOptions: {
        output: {
          manualChunks: {
            vue: [
              'vue', 'vue-router', 'pinia', 'vue-i18n', '@vueuse/core'
            ],
            antd: [
              'ant-design-vue',
      ...
      // ❌ REMOVE Rolldown-specific options
      // rolldownOptions: { ... }  ← delete this entire section
    },

    experimental: {
      useViteNative: false,   // 🔥 This alone disables Rolldown
    },
